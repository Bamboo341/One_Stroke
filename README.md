...
- **有向グラフ**: チェックを入れると線に向き（矢印）が付きます。
  詳しくは [有向グラフモード](#有向グラフモード矢印つきの線) を参照
- **判定の行**: 図形を編集するたびに自動で更新されます。ボタンを押す必要はありません
- **点の色**: 青い縁が普通の点、**赤い縁は「問題のある点」**
  （通常モードでは線が奇数本つながっている「奇数点」）。
  オレンジは線を引くために選択中の点です
...
> **補足**: 番号どうしが重なりそうな場所（線が交差しているところなど）では、
> 番号が自動で線に沿って少しずれて表示されます。番号は必ず自分の線の上に
> 乗っているので、どの線の番号かは見た目どおりです。

### 有向グラフモード（矢印つきの線）

「一方通行の道だけで一筆書きできるか？」を調べたいときは、
右パネルの **「有向グラフ（矢印）」** にチェックを入れます。

![有向グラフモードの例（三角形）](docs/images/screenshot_directed.svg)

- すべての線に**矢じり**が表示され、線は**クリックした順**
  （1点目 → 2点目）の向きにしか進めなくなります
- 判定のルールも変わります。各点で「入ってくる線」と「出ていく線」の
  本数がつり合っていることが条件になり、つり合っていない点が赤い縁で
  表示されます
- **線の向きを変えたいとき**は、削除モードでその線を消してから、
  逆の順番でクリックして引き直してください
- チェックを外せばいつでも普通の（向きのない）モードに戻ります。
  **描いた図形はそのまま残ります**
...
線に向きがある場合（有向グラフモード）は条件が少し変わります。

- すべての点で「入る本数 = 出る本数」→ 可能（閉路）
- 「出る方が1本多い点」と「入る方が1本多い点」が1つずつ → 可能。
  出る方が多い点から始めて、入る方が多い点で終わる
- それ以外 → できない
...
- 判定はオイラーの定理（連結 かつ 奇数点が0個または2個。
  有向モードでは出入次数の均衡）、
  探索は Hierholzer 法のスタック反復実装（O(V+E)、多重辺・有向対応）です
- テストの実行:
